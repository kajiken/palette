---
allowed-tools: Bash(git checkout:*), Bash(git branch:*), Bash(git add:*), Bash(git status:*), Bash(git commit:*), Bash(git diff:*), Bash(git log:*), AskUserQuestion
description: Create commits with appropriate granularity and messages
---
...
Based on the above changes, create commits with appropriate granularity and messages. Follow these steps:
...
1. The list of commits to be created (with messages and files)
...
After user approval:
1. Stage the appropriate files for each commit
2. Create commits as planned using HEREDOC format for messages

### Step 6: Result Display

After successful completion, display:
1. A summary of all commits that were created
...
- Do not push to remote - this command only creates local commits
